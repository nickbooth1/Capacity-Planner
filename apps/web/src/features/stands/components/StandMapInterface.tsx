...
export const StandMapInterface: React.FC<StandMapInterfaceProps> = React.memo(
  ({ stands, selectedStandId, onStandSelect, filters }) => {
    ...
    return (
      <div
        className={`relative bg-white rounded-lg shadow-sm border ${isFullscreen ? 'fixed inset-0 z-50' : ''}`}
        role="region"
      ...
      >
        ...
        {/* Map Legend */}
        <div className="p-4 border-t bg-gray-50" role="complementary" aria-label="Map legend">
          ...
          <div className="hidden md:block mt-3 pt-3 border-t border-gray-200">
            <p className="text-xs text-gray-500">
              Keyboard shortcuts:{' '}
              <kbd className="px-1 py-0.5 bg-gray-100 border rounded text-xs">Ctrl+R</kbd> Reset
              view •
              <kbd className="px-1 py-0.5 bg-gray-100 border rounded text-xs ml-2">Ctrl+F</kbd>{' '}
              Fullscreen •
              <kbd className="px-1 py-0.5 bg-gray-100 border rounded text-xs ml-2">Esc</kbd> Exit
              fullscreen
            </p>
          </div>
        </div>
...
StandMapInterface.displayName = 'StandMapInterface';
